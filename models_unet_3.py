...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels, features):
        ...
        self.enc5 = self._conv_block(features * 8, features * 16, 3, 2, 1)

        # Decoder
        self.dec0 = self._tconv_block(features * 16, features * 8, 4, 2, 1)  # New layer
        self.dec1 = self._tconv_block(features * 16, features * 8, 4, 2, 1)  # Input: [features*8, 4, 4], Output: [features*4, 8, 8]
        self.dec2 = self._tconv_block(features * 12, features * 4, 4, 2, 1)  # Input: [features*8, 8, 8], Output: [features*4, 16, 16]
        self.dec3 = self._tconv_block(features * 6, features * 2, 4, 2, 1)  # Input: [features*6, 16, 16], Output: [features*2, 32, 32]
        self.dec4 = self._tconv_block(features * 3, features * 1, 4, 2, 1)  # Input: [features*3, 16, 16], Output: [features, 64, 64]
    ...
    def forward(self, x):
        ...
        enc5 = self.enc5(enc4)  # New layer

        # Decoding path
        dec0 = self.dec0(enc5)  # New layer
        dec0 = torch.cat((dec0, enc4), dim=1)
        ...
        dec4 = self.dec4(dec3)  # New layer
        dec4 = torch.cat((dec4, x), dim=1)
        out = self.dec_out(dec4)
        return out
